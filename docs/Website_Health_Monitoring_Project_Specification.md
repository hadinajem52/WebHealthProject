...
| BR-C02 | The system records subject, issuer, serial, SHA-256 fingerprint, valid-from, valid-to, days remaining and separate validity, hostname and chain-trust facts when available. | Certificate history supports renewal comparison without storing certificate or private-key bytes. Chain status uses bounded canonical names; ambiguous historical trust is Unknown. Revocation is not checked and certificate-controlled downloads are disabled. |
| BR-C03 | An expired, not-yet-valid, hostname-mismatched, untrusted or handshake-failing certificate is critical.           | Expiry, hostname, trust and not-yet-valid findings are independent; primary precedence is not-yet-valid, expired, hostname mismatch, untrusted, expiring soon.                                |
| BR-C04 | Expiry severity defaults to warning at 30 days, high at 15 days and critical at 7 days.                           | Boundary dates produce the expected severity and alert; thresholds require warning > high > critical >= 0.                       |
...
| BR-L02 | robots.txt is respected unless the run requests an override; manual runs request one on every crawl (owner decision, 2026-08-24). Every run records whether it bypassed robots. | A broken link behind a `Disallow` is still reported; the bypass is never silent. |
