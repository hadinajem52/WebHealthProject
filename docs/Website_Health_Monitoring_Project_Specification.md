...
| BR-L02 | robots.txt is respected unless the run requests an override; manual runs request one on every crawl (owner decision, 2026-08-24). A crawl still reaches only hosts with recorded target-authorization evidence, and every run records whether it bypassed robots. | A broken link behind a `Disallow` is still reported; the bypass is never silent. |
