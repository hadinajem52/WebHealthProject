...
Phase 6 runtime addition reviewed on 2026-08-19: `AngleSharp` `1.7.1`, for SEO value extraction (BR-E01, BR-E10). It was selected over regular expressions, which are both a correctness and a denial-of-service trap over untrusted HTML, and over a hand-written scanner, which would move HTML5 tokenising risk into this repository. It resolves with **no transitive dependencies** on `net10.0` — verified with `dotnet list package --include-transitive` against a throwaway probe project — so it adds one MIT-licensed assembly rather than a graph. It is referenced only by `WebHealth.Infrastructure`, pinned centrally, and locked in that project's `packages.lock.json`; `dotnet list WebHealthProject.sln package --vulnerable --include-transitive` reports nothing new (the only entry remains the accepted `SSH.NET` advisory above). The parser is used without a browsing context or requester, so it cannot fetch anything a parsed document references. The full decision is recorded in [`../phase-6/SEO_Value_Extraction.md`](../phase-6/SEO_Value_Extraction.md).

PNG audit Increment 1 runtime addition reviewed on 2026-08-26: `SixLabors.ImageSharp` `3.1.12` is referenced only by `WebHealth.Infrastructure`, pinned centrally, and locked through the affected project graphs. Locked restore succeeds and the vulnerability audit reports no ImageSharp advisory; the only reported advisory remains the accepted Testcontainers `SSH.NET` advisory above. ImageSharp uses the Six Labors Split License 1.0, so the license must be reviewed again if the project's personal portfolio profile or dependency version changes. The dependency, license, APNG limitation, fixtures and comparison proof are recorded in [`../General/PNG_Image_Audit_Operations_and_Decisions.md`](../General/PNG_Image_Audit_Operations_and_Decisions.md).
